...
const Products = () => {
  ...
  const fetchComments = async (props) => {
    const res = await fetch(
      `http://localhost:3001/api/product?&category=${props.category}&offset=${props.currentPage}`
    );
    const items = await res.json();
    return items;
  };
  ...
  const fetchComments = async (props) => {
    const res = await fetch(
      `http://localhost:3001/api/product?&category=${props.category}&offset=0`
    );
    const items = await res.json();
    return items;
  };
